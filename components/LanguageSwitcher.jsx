  ...
  return (
    ...
    <div className="relative inline-block w-20 h-8">
      ...
      <div className="relative flex items-center h-full px-2">
        <span className={`text-xs font-bold ${router.locale === 'en' ? 'text-black dark:text-white' : 'text-gray-200 dark:text-gray-400'}`}>
          EN
        </span>
        <span className={`text-xs font-bold ml-auto ${router.locale === 'hi' ? 'text-white dark:text-black' : 'text-gray-400 dark:text-gray-200'}`}>
          HI
        </span>
      </div>
